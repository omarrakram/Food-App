...
| **3P.4. Semantics, not scores** | Wholemeal flour and wheat bran earned canonical rows on their own merits and `eish-baladi` came off hold as a real recipe; `eggah-bel-batates` was RETIRED as a resolved duplicate rather than parked or blamed on its photograph; two batch-4 cuisine claims were audited and `shorbet-2ar3` moved OUT of Egyptian; `spaghetti-bolognese` renamed to what it is | Found a shipped bug nobody had read: `بطاطس` — the ordinary Arabic word for potato — resolved to **frozen chips**, because the importer's collision gate never checked `nameAr`. The coverage audit had been calling `potatoes` ambiguous for exactly this reason. Egyptian **59 → 59** after the audit; the metric was not defended |
| **5. Batch 5** | 28 candidates aimed at lamb, seafood, short recipes and breakfast. 18 staged, 10 found nothing, **4 approved**, 13 refused, 1 held. The closest call yet: `kabab-mashwi`'s photograph is a good one of MINCED kebab where the candidate is cubed beef, and rewriting the candidate to fit would have let the photograph choose the dish | 200 → **205** recipes; photo coverage 53.0% → **54.1%**; ≤5-line **28 → 29** (from 3P.4's bread, not from batch 5); Egyptian 59 → **61** (29.8%) |
...
| Stage | Work | Measured exit condition |
| --- | --- | --- |
| **3. Recipes, batch by batch** (in progress, 5 of ~6 batches done) | Each batch is: declare the dish identities → run the candidate acquisition → a human reviews `PHOTO_CANDIDATE_REVIEW.md` → write recipes for the dishes that survived → promote their photographs in the same batch. A dish may also be **held** (photograph fine, dish not writable yet) or **retired** (the concept is already taken) | Per batch: photo coverage **never below 41.6%**, and rising toward 60% (**54.1%** after batch 5). Overall: ≤5-ingredient recipes ≥44 (**29**); Egyptian ≥30% (**29.8%**); recipes ~300 (**205**) |
| **5. Prices** | The ranked backlog in `PRICE_BACKLOG.md`, re-ranked after each recipe batch because new recipes change which gaps block an estimate | Slot coverage **≥95%** (**77.1%** after batch 5; the top 50 of 115 reach 93.2%) |
